...
def parse_arduino_line(line):
    ...
    Extraemos el carácter decodificado.
    """
            ...
            if state['serial'] and state['connected']:
                if state['serial'].in_waiting > 0:
                    ...
                    if char:
                        ...
                        socketio.emit('char_received', {'char': char, 'buffer': state['char_buffer']})

                        # Detectar fin de mensaje (salto de línea = ASCII 10 transmitido)
                        if char == '\n' or raw.endswith('\\n') or state['char_buffer'].endswith('\n'):
                    ...
                    # Mensajes de calibración/estado
...
def flush_buffer():
    """Permite forzar el guardado del buffer actual como mensaje."""
    msg_text = state['char_buffer'].strip()
    if msg_text:
        entry = {
            'id': len(state['history']) + 1,
